...
/**
 * Centralised bipartite matching algorithm:
 * Assign empty agents to waiting resources for every t seconds. The time interval t is 10 seconds by default.
 */
public class AgentBipartiteMatching extends BaseAgent {
    ...
    /**
     * AgentBipartiteMatching constructor.
     *
     * @param id An id that is unique among all agents and resources
     * @param map The map
     */
    public AgentBipartiteMatching (long id, CityMap map) {
        super(id, map);
        rnd = new Random(id);
        if (dataModel == null) {
