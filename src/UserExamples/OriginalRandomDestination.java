 ...
 */
public class OriginalRandomDestination extends BaseAgent {
     ...
     * @param map The map
     */
    public OriginalRandomDestination(long id, CityMap map) {
        super(id, map);
        rnd = new Random(id);
        if (dataModel == null) {
    ...
    public void planSearchRoute(LocationOnRoad currentLocation, long currentTime) {
        ...
        Intersection[] intersectionArray = map.intersections().values().toArray(new Intersection[0]);
